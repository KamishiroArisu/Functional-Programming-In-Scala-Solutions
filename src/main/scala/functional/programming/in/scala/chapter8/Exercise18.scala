...
object Exercise18 {
    def takeWhileProp[A](g: Gen[A])(p: A => Boolean) = {
        val sg = SGen.listOf(g)
        val p1 = (l: List[A]) => l.takeWhile(p) ++ l.dropWhile(p) == l
        val p2 = (l: List[A]) => l.takeWhile(p).forall(p)
        val p3 = (l: List[A]) =>
            l.slice(l.takeWhile(p).length, l.length).headOption match {
                case Some(a) => !p(a)
                case _ => true
            }

        forAll(sg)(l => p1(l) && p2(l) && p3(l))
    }
